...
class Autosampler():
    ...
    def initialize(self):
        ...
        if (self.ver.startswith('Cetac')):
            ...
            self.commandOut('UP')
            self.commandOut('ABS=0-0-0')
            self.commandOut('DOWN=40')
            self.commandOut('PMP OFF')
            return True
        else:
            return False
    ...
    def goto(self,rack,x,y,z=None,wait=True):
        ...
        if z==None:
            ...
            print(f'Probe Depth: {z}')
        colWidth = 189
        colOffset= 20
        rackWidth = 1098
        rowHeight = 194
        rowOffset = 510
        print(f'x={x} y={y} rack={rack}')
            ...
            if self.name == 'ASX-520':
                ...
                while True:
                    ...
                    try:
                        ...
                        print(f'ValueError in GETPOS - {e}')

    def gotoRinse(self):
        print("GOTORINSE CALLED")
        self.commandOut(f'ABS=0-0-100') 

    def query(self,q):
        qout = q +self.EOL
        if (self.DEBUG):
            print(f'Writing {qout}')
    ...
    def commandOut(self,command):
        ...
        if self.DEBUG:
            print(f'Writing to {self.name}: {writeCommand}')
